  ...
  return (
        <article className={` ${siteConfig('HEO_POST_LIST_COVER_HOVER_ENLARGE', null, CONFIG) ? ' hover:scale-110 transition-all duration-150' : ''}`} >
            ...
            <div data-wow-delay=".2s"
                ...
                {/* 图片封面 */}
                {showPageCover && (
                    // <Link href={url} passHref legacyBehavior target="_blank">
                    <Link href={url} passHref target="_blank">
                        <div className="w-full md:w-5/12 2xl:w-full overflow-hidden">
                            <LazyImage priority={index === 0} src={post?.pageCoverThumbnail} alt={post?.title} className='h-60 w-full object-cover group-hover:scale-105 group-hover:brightness-75 transition-all duration-300 cursor-pointer' />
                        </div>
                    </Link>
                )}
